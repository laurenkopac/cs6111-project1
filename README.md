...
4. After scores are assigned to all terms, we reorder them using `intertools` and `permutations` to generate all possible arrangements and determine the highest scoring. [5] see section below for additional details on how the `reorder_query()` method works.

### Reordering Additional Details
1. Generate all permutations of the new query to represent the reorder options ('restaurant per se michelin', 'michelin restaurant per se',
'per se restaurant michelin', ...)
2. Generate bigrams of the permutations. for example one permutation would be [per, se, restaurant, michelin] and the bigrams [(per, se), (se, restaurant), and so on] using CountVectorizer
3. Generate the tf-idf weights of each bigrams and add up the weighting for each bigram and save to that permutation using TfIdfVectorizer
4. Find the best permutation by the top weight score and that should be the new augmented query. (per se restaurant michelin) 
